...
PORTAS="22"
#LIBERAR PORTA TCP/UDP
PORTASL=`cat /etc/init.d/lport/lport.txt`
PROTOCOL=`cat /etc/init.d/lport/protocol.txt`
#TCP
TCP=`cat /etc/init.d/lport/tcp.txt`
#UDP
...
/sbin/iptables -A INPUT -f -j LOG --log-prefix "Pacote INPUT fragmentado: " # gera log de pacotes fragmentados
    ...
    #Liberar Porta TCP
    for lt in $TCP;do
    /sbin/iptables -A INPUT -p tcp --dport $lt -j ACCEPT
    done

    #Liberar Porta UDP
    for lu in $UDP;do
    /sbin/iptables -A INPUT -p tcp --dport $lu -j ACCEPT
    done
